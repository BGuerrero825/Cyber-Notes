...
We already loaded kernel32 the hard way, which gives us access to it's functions, like LoadLibraryA, which can be used to load ws2_32.dll and its functions. Its functions can then be found using the same method we built before (or alternatively GetProcAddress also from kernel32).
